一个简单的自动化测试脚本，该脚本主要有5个包，分别是：ActivityObject, Base, Doc, Pub_method, TestCase
...
Doc: 存放测试数据，和一些配置文件的包。
TestCase: 存放测试用例的包。
除了上面的包，还有result和report两个包，result存放的是allure报告生成的原始数据，report中生成的是allure生成的html报告
